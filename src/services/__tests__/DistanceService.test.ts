...
describe('DistanceService', () => {
  ...
  test('The sort parkruns function returns the same parkrun FeatureCollection when providing parkruns with no distanceToAddress attribute.', () => {
    const parkruns = DistanceService.sortParkruns(
      mockedParkrunFeatureCollection
    );
  ...
  test('The sort parkruns function returns a FeatureCollection in ascending order of features using the distanceToAddress attribute.', () => {
    const parkruns = DistanceService.sortParkruns(
      mockedParkrunFeatureCollectionWithDistances
    );
  ...
  test('The NUM_NEAREST (defined as a constant) closest parkruns are identified when given a sorted parkrun FeatureCollection.', () => {
    const parkruns = DistanceService.setNearestParkrunAttributes(
      mockedSortedParkrunFeatureCollection
    );
    const nearestParkruns = parkruns.features.filter(
      (f) => f.properties.parkrunClose === 'true'
    );
